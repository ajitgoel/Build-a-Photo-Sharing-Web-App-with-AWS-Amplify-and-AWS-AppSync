...
const S3ImageUpload = (props) => {
  ...
  const uploadFile = async (file) => {
    //We prefix the photo name with ‘upload/’ because we’ll want to automatically make thumbnails for each image.
    ...
    const result = await Storage.vault.put(
      ...
      {
        metadata: {
          //We pass in metadata: { albumid: this.props.albumId } because we’re going to have an S3 thumbnail trigger function take care of 
          //adding the information about this photo to our data store after it finishes making the thumbnail, and that function will 
          //somehow need to know what album the photo was uploaded for.
